  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        setSuccessMsg('Master Distiller access verified.');
        handleAdminCmsClick();
        return;
      }
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 sm:p-6">
        ...
        >
            ...
            {activeTab === 'login' && (
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 px-4 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 active:scale-[0.99] disabled:opacity-50 text-stone-950 font-bold text-sm rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20 cursor-pointer mt-4"
                >
          ...
          <div className="p-4 border-t border-stone-800 bg-stone-950/80 flex items-center justify-center text-xs text-stone-400">
            <button
              onClick={closeAuthModal}
              className="text-stone-400 hover:text-amber-400 underline underline-offset-4 transition cursor-pointer font-medium py-1 px-3 rounded-lg hover:bg-stone-900"
            >
              Continue as Guest
            </button>
          </div>
